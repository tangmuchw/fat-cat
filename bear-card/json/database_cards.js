const path = require("path");
const fs = require("fs");
const yargs = require("yargs/yargs");
const moment = require("moment");
const { hideBin } = require("yargs/helpers");
const { createDirSync, getDevCloudUrl } = require("./utils");
const { CARD_CATEGORY_TYPES, CARD_TYPES_DICTIONARIES } = require("./constants");
const { getCardsFromBBearCardExcel } = require("./parse_excel");

// --env=dev|prod
const argv = yargs(hideBin(process.argv)).argv;
const env = argv["env"] || "dev";
console.log(`=== database_cards.js argv env: ${env} ===`); // 例如：node script.js --name=John --age=

// console.log(`=== 卡片类型: ${CARD_TYPES_DICTIONARIES.length} 种 ===`);
// console.log(CARD_TYPES_DICTIONARIES.map(({ name }) => name).join("、"));
// console.table(CARD_TYPES_DICTIONARIES);

// // 记得先创建 data 目录

// const defaultFiveThemeCards = [
//     {
//         categoryType: "HoneyPocket",
//         type: "thanks",
//         templateCode: "thanks0000",
//         templateName: "万笺集",
//         cover: "thanks0000.png",
//         salePrice: null,
//         content: "千言万语汇成一句：谢谢！❤️",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "sorry",
//         templateCode: "sorry0000",
//         templateName: "万笺集",
//         cover: "sorry0000.png",
//         salePrice: null,
//         content: "为我的不当言行向您郑重道歉！🙏",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "encourage",
//         templateCode: "encourage0000",
//         templateName: "万笺集",
//         cover: "encourage0000.png",
//         salePrice: null,
//         content: "关关难过关关过，前路漫漫亦灿灿！🔥",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "reject",
//         templateCode: "reject0000",
//         templateName: " 万笺集",
//         cover: "reject0000.png",
//         salePrice: null,
//         content: "很抱歉这次帮不上忙，但真心祝愿一切顺利！🙏",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "praise",
//         templateCode: "praise0000",
//         templateName: "万笺集",
//         cover: "praise0000.png",
//         salePrice: null,
//         content: "救命！这是什么神仙操作！慕了慕了～",
//         updatedAt: null,
//     },
// ];

// const thanks = [
//     // 感谢卡
//     {
//         categoryType: "ResilienceHive",
//         type: "thanks",
//         templateName: "改PPT之恩",
//         cover: "thanks0001.png",
//         salePrice: null,
//         content: "深夜改PPT之恩，没齿难忘！",
//         updatedAt: null,
//     },
//     {
//         categoryType: "StarlightAtlas",
//         type: "thanks",
//         templateName: "带饭之恩",
//         cover: "thanks0002.png",
//         salePrice: null,
//         content:
//             "从今以后，你的备注是【再生父母·食堂在逃天使·饿鬼拯救者】！请受我一拜！ORZ",
//         updatedAt: null,
//     },
//     {
//         categoryType: "ResilienceHive",
//         type: "thanks",
//         templateName: "职场新人致谢导师",
//         cover: "thanks0003.png",
//         salePrice: null,
//         content: "新手村遇名师，何其有幸！您说的‘[导师金句]’已刻进职场DNA🧬",
//         updatedAt: null,
//     },
//     {
//         categoryType: "AmygdalaCourt",
//         type: "thanks",
//         templateName: "致疲惫时刻",
//         cover: "thanks0004.png",
//         salePrice: null,
//         content: "连续肝了3天pre，没猝死就是胜利！奖励一顿炸鸡！",
//         updatedAt: null,
//     },
//     {
//         categoryType: "NeuroBlossom",
//         type: "thanks",
//         templateName: "谢谢你像蜂蜜一样",
//         cover: "thanks0005.png",
//         salePrice: null,
//         content: "谢谢你像蜂蜜一样，把普通的日子都酿得甜丝丝！🐝🍯✨",
//         updatedAt: null,
//     },
//     {
//         categoryType: "NeuroBlossom",
//         type: "thanks",
//         templateName: "超级幸运日",
//         cover: "thanks0006.png",
//         salePrice: null,
//         content: `🧺 偷偷告诉你～”和你一起啃玉米的日子，是小熊日历里标 🌟 的“超级幸运日”！🌽🐻💛`,
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "thanks",
//         templateName: "转动引力",
//         cover: "thanks0007.png",
//         salePrice: null,
//         content: `🪐 渺小星球相遇，谢谢你转动引力靠近我！`,
//         updatedAt: null,
//     },
// ];

// // 鼓励卡
// const encourages = [
//     {
//         categoryType: "NeuroBlossom",
//         type: "encourage",
//         templateName: "反焦虑",
//         cover: "encourage0001.png",
//         salePrice: null,
//         content: "怕什么？最坏的结果，不过是大器晚成。",
//         updatedAt: null,
//     },
//     {
//         categoryType: "StarlightAtlas",
//         type: "encourage",
//         templateName: "备考打气",
//         cover: "encourage0002.png",
//         salePrice: null,
//         content: "凌晨的台灯和咖啡渍，都是未来讲给学弟妹的勋章故事✨",
//         updatedAt: null,
//     },
//     {
//         categoryType: "NeuroBlossom",
//         type: "encourage",
//         templateName: "勇敢熊熊不怕困难",
//         cover: "encourage0003.png",
//         salePrice: null,
//         content: "别怕！小熊盾牌借你！跌倒了？爬起来更酷！(ง •̀_•́)ง💥",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "encourage",
//         templateName: "蜂蜜罐罐哲理",
//         cover: "encourage0004.png",
//         salePrice: null,
//         content: `🍯 有时像蜂蜜一样黏糊糊也没关系～`,
//         updatedAt: null,
//     },
//     {
//         categoryType: "CouragePawFolios",
//         type: "encourage",
//         templateName: "进步小天才认证",
//         cover: "encourage0005.png",
//         salePrice: null,
//         content: `每一步都算数～超棒的！奖励小熊花花一朵！🌼🐻💖`,
//         updatedAt: null,
//     },
// ];

// // 夸夸卡
// const praises = [
//     {
//         categoryType: "NeuroBlossom",
//         type: "praise",
//         templateName: "认真搞事业",
//         cover: "praise0001.png",
//         salePrice: null,
//         content: "你认真搞事业的样子，自带主角光环！",
//         updatedAt: null,
//     },
//     {
//         categoryType: "NeuroBlossom",
//         type: "praise",
//         templateName: "朋友彩虹屁",
//         cover: "praise0002.png",
//         salePrice: null,
//         content:
//             "与君初相识，犹如故人归——原来上辈子我们就是互吹彩虹屁的搭档啊！",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "praise",
//         templateName: "笑容甜度超标",
//         cover: "praise0003.png",
//         salePrice: null,
//         content: "你的笑容甜度超标啦～像刚偷吃了蜂蜜罐罐！(๑˃ᴗ˂)ﻭ ✧",
//         updatedAt: null,
//     },
//     {
//         categoryType: "CouragePawFolios",
//         type: "praise",
//         templateName: "熊熊认证：超厉害勋章",
//         cover: "praise0004.png",
//         salePrice: null,
//         content: `🏆 “哇！你也太棒了吧！” 连小熊都忍不住要给你鼓掌爪爪！👏🐾 (ง •̀_•́)ง 超强！`,
//         updatedAt: null,
//     },
//     {
//         categoryType: "NeuroBlossom",
//         type: "praise",
//         templateName: "温柔击中",
//         cover: "praise0005.png",
//         salePrice: null,
//         content: `💖 糟糕！被你的温柔击中啦～`,
//         updatedAt: null,
//     },
// ];

// // 道歉卡
// const sorrys = [
//     {
//         categoryType: "HoneyPocket",
//         type: "sorry",
//         templateName: "鸽了饭局",
//         cover: "sorry0001.png",
//         salePrice: null,
//         content: "鸽了饭局，欠你一顿火锅+奶茶。",
//         updatedAt: null,
//     },
//     {
//         categoryType: "AmygdalaCourt",
//         type: "sorry",
//         templateName: "吵架和解",
//         cover: "sorry0002.png",
//         salePrice: null,
//         content: "吵架时说反话的我，其实超怕你转身走掉…",
//         updatedAt: null,
//     },
//     {
//         categoryType: "AmygdalaCourt",
//         type: "sorry",
//         templateName: "帮你贴好难过",
//         cover: "sorry0003.png",
//         salePrice: null,
//         content: "能给我一块创可贴，帮你贴好难过吗？",
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "sorry",
//         templateName: "我的粗心",
//         cover: "sorry0004.png",
//         salePrice: null,
//         content: `🧸 “你值得被温柔包围，而不是我的粗心…”`,
//         updatedAt: null,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "sorry",
//         templateName: "爪印认罪书",
//         cover: "sorry0005.png",
//         salePrice: null,
//         content: `📜 “小熊议会严肃通告：”第 𝟭 条：承认错误！第 𝟮 条：超级后悔！第 𝟯 条：努力改正！ 请验收！`,
//         updatedAt: null,
//     },
// ];

// // 拒绝卡
// const rejects = [
//     {
//         categoryType: "HoneyPocket",
//         type: "reject",
//         templateName: "你超优秀",
//         cover: "reject0001.png",
//         salePrice: null,
//         content: "你超优秀，但我怕耽误你发展。",
//         updatedAt: null,
//     },
//     {
//         categoryType: "AmygdalaCourt",
//         type: "reject",
//         templateName: "拒绝借钱",
//         cover: "reject0002.png",
//         salePrice: null,
//         content: "我现在的经济状况：苍蝇飞过都得留下买路钱，实在薅不出羊毛了🐑",
//         updatedAt: null,
//     },
//     {
//         categoryType: "ResilienceHive",
//         type: "reject",
//         templateName: "紧急会议中",
//         cover: "reject0003.png",
//         salePrice: null,
//         content:
//             "超想和你一起啃玉米的！但今天小爪爪堆满了任务 ⏰ 罚我下次带双倍蜂蜜茶赔罪！ ( ‘-ωก̀ )🌽✨",
//         updatedAt: null,
//     },
//     {
//         categoryType: "AmygdalaCourt",
//         type: "reject",
//         templateName: "守卫蜂蜜罐边界",
//         cover: "reject0004.png",
//         salePrice: null,
//         content:
//             "再好的朋友也要尊重对方的蜂蜜罐边界！🍯❌ 这次请原谅我守卫小罐子的爪爪～ (๑•́ ₃ •̀๑)",
//         updatedAt: null,
//     },
//     {
//         categoryType: "ResilienceHive",
//         type: "reject",
//         templateName: "爪爪满啦~拒接新任务",
//         cover: "reject0005.png",
//         salePrice: null,
//         content:
//             "熊熊爪爪已堆满松果啦！为保证每颗果子都香甜，新任务 D 可能要先放树洞排队哦~ 我们一起看看哪颗最急？",
//         updatedAt: null,
//     },
// ];

// // 纪念卡
// const souvenirs = [
//     {
//         categoryType: "HoneyPocket",
//         type: "souvenir",
//         templateName: "往返车票",
//         cover: "souvenir0001.png",
//         salePrice: null,
//         content: "用一张车票换走学生证，从此故乡只有冬夏",
//         updatedAt: null,
//         status: "offline",
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "souvenir",
//         templateName: "致青春",
//         cover: "souvenir0002.png",
//         salePrice: null,
//         content: "愿你此去前程似锦，再无风雨兼程，归来仍是少年，眼中满是星辰。",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
// ];

// // 自嘲卡
// const selfMockeries = [
//     {
//         categoryType: "HoneyPocket",
//         type: "selfMockery",
//         templateName: "天选打工人工资篇",
//         cover: "selfMockery0001.png",
//         salePrice: null,
//         content: "工资三千八，拿命往里搭",
//         updatedAt: null,
//         status: "offline",
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "selfMockery",
//         templateName: "天选打工人凌晨篇",
//         cover: "selfMockery0002.png",
//         salePrice: null,
//         content: "凌晨四点，看到海棠花未眠",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
// ];

// const embraces = [
//     {
//         categoryType: "HoneyPocket",
//         type: "embrace",
//         templateName: "抱紧我",
//         cover: "embrace0001.png",
//         salePrice: null,
//         content: "抱紧我，让心跳缝补风的缺口。",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
//     {
//         categoryType: "HoneyPocket",
//         type: "embrace",
//         templateName: "「无声胜有声」安慰术",
//         cover: "embrace0002.png",
//         salePrice: null,
//         content: "熊熊不说大道理，陪你安静看月亮升起…",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
// ];

// // 充能卡
// const recharges = [
//     {
//         categoryType: "ResilienceHive",
//         type: "recharge",
//         templateName: "申请成长加油包",
//         cover: "recharge0001.png",
//         salePrice: null,
//         content:
//             "过去一年熊熊努力囤了 365 罐蜂蜜！想给树屋换个更大蜜柜~ 能聊聊熊熊的蜂蜜价值吗？",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
// ];

// // 轻效卡
// const efficiencies = [
//     {
//         categoryType: "ResilienceHive",
//         type: "efficiency",
//         templateName: "熊掌快会",
//         cover: "efficiency0001.png",
//         salePrice: null,
//         content:
//             "最近会议像熊群追尾巴~ 要不要筛出核心坚果 🌰？省下时间多囤冬粮！⏳",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
// ];

// //  任务卡
// const tasks = [
//     {
//         categoryType: "SimmeringHearth",
//         type: "task",
//         templateName: "家务刺客·善后指南",
//         cover: "task0001.png",
//         salePrice: null,
//         content:
//             "锅盖油油在求救！砧板哭诉被冷落～调味瓶变身‘小油瓶’✨<br>熊熊紧急出动！死角油渍退散！<br>任务完成记得求夸夸‘家务王者’哦～ 💪",
//         updatedAt: null,
//         status: "online",
//         isHot: 0,
//     },
// ];

// const generateTemplateCode = (list) => {
//     return list.map((v, idx) => ({
//         ...v,
//         templateCode: `${v.type}${(idx + 1).toString().padStart(4, "0")}`,
//     }));
// };

// const cards = [
//     ...defaultFiveThemeCards,
//     // 分割--以上为默认模板
//     ...generateTemplateCode(thanks),
//     ...generateTemplateCode(encourages),
//     ...generateTemplateCode(praises),
//     ...generateTemplateCode(sorrys),
//     ...generateTemplateCode(rejects),
//     ...generateTemplateCode(souvenirs),
//     ...generateTemplateCode(selfMockeries),
//     ...generateTemplateCode(embraces),
//     ...generateTemplateCode(recharges),
//     ...generateTemplateCode(efficiencies),
//     ...generateTemplateCode(tasks),
// ];

// // const formatDate = (date) => {
// //     const year = date.getFullYear();
// //     const month = String(date.getMonth() + 1).padStart(2, "0");
// //     const day = String(date.getDate()).padStart(2, "0");
// //     const hours = String(date.getHo
// // urs()).padStart(2, "0");
// //     const minutes = String(date.getMinutes()).padStart(2, "0");
// //     const seconds = String(date.getSeconds()).padStart(2, "0");

// //     return `${year}-${month}-${day}${hours}:${minutes}:${seconds}`;
// // };

const cloudUrlConfig = {
    dev: getDevCloudUrl(),
    // preprod:
    //     "cloud://cloud1-4gcgkweic0306525.636c-cloud1-4gcgkweic0306525-1360400413/images/cover/",
    prod: "cloud://prod-env-3gb9vy5c3ca5a90c.7072-prod-env-3gb9vy5c3ca5a90c-1360400413/images/cover/",
};

const fileNameConfig = {
    dev: "dev_database_cards",
    preprod: "preprod_database_cards",
    prod: "prod_database_cards",
};

// const cardCoverRatios = ["562x375", "676x338"];

// !important
const cloudUrl = cloudUrlConfig[env];
const fileName = fileNameConfig[env];

const nowAt = moment();
const dateStr = nowAt.format("YYYY-MM-DDTHH:mm:ss.SSSZ");
const updatedAt = { $date: dateStr };

// console.log(`=== 更新时间: ${updatedAt.$date} ===`);
// console.log(`=== 卡片分类类型: ${CARD_CATEGORY_TYPES.length} 种 ===`);
// console.table(CARD_CATEGORY_TYPES);

// const updatedCards = cards.map((v, idx) => {
//     const id = v.templateCode;

//     const { categoryTypeName } =
//         CARD_CATEGORY_TYPES.find(
//             ({ categoryType }) => categoryType === v.categoryType
//         ) || {};
//     const { name: typeName } =
//         CARD_TYPES_DICTIONARIES.find(({ value }) => value === v.type) || {};

//     // const coverUrl = v.cover ? `${cloudUrl}${v.cover}` : "";
//     // const d = cardCoverRatios.reduce((a, ratio) => {
//     //     return {
//     //         ...a,
//     //         [ratio]: v.cover ? `${cloudUrl}${ratio}_${v.cover}` : "",
//     //     };
//     // }, {});
//     return {
//         // id: "202505191212120002",
//         _id: id,
//         ...v,
//         typeName,
//         categoryTypeName,
//         status: v.status || "online",
//         createAt: null,
//         updatedAt,
//         cover: v.cover ? `${cloudUrl}${v.cover}` : null,
//         isHot: v.isHot || 0,
//         // copywriting: v.copywriting || "永久有效 | 编号唯一",
//     };
// });

// const hotCards = updatedCards
//     .filter(({ isHot }) => isHot === 1)
//     .map(
//         ({ typeName, templateName, content }) =>
//             `${typeName}·${templateName}·${content}`
//     );

// const initialCardCategoryCount = CARD_CATEGORY_TYPES.reduce(
//     (a, { categoryType }) => {
//         return {
//             ...a,
//             [categoryType]: 0,
//         };
//     },
//     {}
// );

// // 统计各个大主题下卡片的数量
// const statsCardCategoryCount = updatedCards.reduce((a, { categoryType }) => {
//     return {
//         ...a,
//         [categoryType]: a[categoryType] + 1,
//     };
// }, initialCardCategoryCount);
// // .join("\n");
// console.log("=== 各个大主题下卡片的数量 ===");
// console.table(statsCardCategoryCount);

// console.log("=== 热门推荐 ===");
// console.table(hotCards);

const DIR_PATH = path.join(__dirname, `cards`);

const getUpdatedCards = async () => {
    const cardsFromBBEarCardExcel = await getCardsFromBBearCardExcel();
    const nextCards = cardsFromBBEarCardExcel.map((v, idx) => {
        return {
            ...v,
            status: v.status || "online",
            createAt: null,
            updatedAt,
            cover: v.cover ? `${cloudUrl}${v.cover}` : null,
            // copywriting: v.copywriting || "永久有效 | 编号唯一",
        };
    });

    return nextCards;
};

const createCardsJson = async () => {
    console.log("=== database_cards 开始处理 ===");
    const filePath = `${DIR_PATH}/${fileName}.json`;
    const writeStream = fs.createWriteStream(filePath);

    const updatedCardsFromExcel = await getUpdatedCards();
    updatedCardsFromExcel.forEach((item) => {
        writeStream.write(JSON.stringify(item) + "\n");
    });
    // updatedCards.forEach((item) => {
    //     writeStream.write(JSON.stringify(item) + "\n");
    // });
    writeStream.end();
    console.log(
        `=== 写入成功-环境 ${env}, 共 ${updatedCardsFromExcel.length} 条 ===`
    );
};

try {
    createDirSync(DIR_PATH);

    createCardsJson();
} catch (err) {
    console.log(`=== 写入失败-环境 ${env} ===`);
}

// 执行: node ./bear-card/json/database_cards.js --env=dev
// 执行: node ./bear-card/json/database_cards.js --env=preprod
// 执行: node ./bear-card/json/database_cards.js --env=prod
