import path from "path";
import fs from "fs";
import yargs from "yargs/yargs";
import { hideBin } from "yargs/helpers";
import { createDirSync } from "./utils";
import moment from "moment";

// --env=dev|prod
const argv: any = yargs(hideBin(process.argv)).argv;
const env: "dev" | "prod" = argv["env"] || "dev";
console.log(`=== database-sent-card-records.js argv env: ${env} ===`); // 例如：node script.js --name=John --age=

const nowAt = moment();
const dateStr = nowAt.format("YYYY-MM-DDTHH:mm:ss.SSSZ");
const nowAtObj = { $date: dateStr };

console.log(`=== 更新时间: ${nowAtObj.$date} ===`);
const record = {
    _id: "testid",
    sentId: "testid",
    cardId: "Thanks-6a7ebcb514b1e354",
    cardNumber: "",
    status: "created",
    recipientOpenid: null,
    senderOpenid: "o7TSC4siUlso9PNAN6PyKh7q79RE",
    createAt: nowAtObj,
    sentAt: null,
    expiredAt: null,
    finalAt: null,
};

const DIR_PATH = path.join(__dirname, `output/sent-card-records`);

const fileName = `${env}-sent-card-records.json`;

const createConfigsJson = () => {
    console.log("=== database-sent-card-records 开始处理 ===");
    const filePath = `${DIR_PATH}/${fileName}`;
    const writeStream = fs.createWriteStream(filePath);

    writeStream.write(JSON.stringify(record) + "\n");
    writeStream.end();
    console.log(`=== 写入成功-环境 ${env} ===`);
};

try {
    createDirSync(DIR_PATH);

    createConfigsJson();
} catch (err) {
    console.log(`=== 写入失败-环境 ${env} ===`, err);
}

// 执行: ts-node ./bear-card/json/src/database-sent-card-records.ts --env=dev
