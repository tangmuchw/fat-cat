...
const cardTypes = {
    ...
    Prophecy: {
        ...
        coverTitle: "预见未来",
        coverSubtitle: "星辰指引你的方向",
        gradient: "prophecy",
        primaryColor: "#7B5BFF", // 更明亮的紫罗兰色
        darkColor: "#5E3EF6", // 稍深的紫色
        lightColor: "#9E7EFF", // 明亮的薰衣草紫
        textColor: "#3A2A7C", // 深紫色文字
        secondaryLight: "#C7B5FF", // 浅紫色
        secondaryDark: "#7B5BFF",
        contentTitle: "我预见你将在事业上取得突破性成就！",
        contentText:
            "根据你展现出的创新思维和坚韧品质，我预见在不久的将来，你将领导一个重要项目并取得突破性成功。你的坚持和智慧会为你打开新的职业道路，星辰为你指引方向，宇宙的力量在支持着你！",
    },
};
...
const generateLessCode = (themes: any) => {
    let lessCode = "";
    ...
    for (const [themeName, variables] of Object.entries(themes)) {
        ...
        // 添加所有变量
        for (const [varName, varValue] of Object.entries(variables as any)) {
...
try {
    ...
    const fileName = `card-type-themes.less`;
    const filePath = path.join(__dirname, `output/${fileName}`);
    const writeStream = fs.createWriteStream(filePath);
    writeStream.write(lessContent);
    writeStream.end();
    log("generate-card-type-theme 成功", lessContent);
} catch (err) {
    console.error(err);
}

// 执行: ts-node ./bear-card/json/src/generate-card-type-theme.ts
