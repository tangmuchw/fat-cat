...
在新的 React 架构中，一个组件的渲染被分为两个阶段 构建 和 commit：
...
> 让你在UI中延迟更新某个不那么重要的部分，与防抖不同，它没有固定延迟，会在主任务完成后进行可中断的渲染

### useTransition

> 用于将非紧急的状态更新（如渲染大量搜索结果）标记为“过渡”任务。这些任务可以被更高优先级的更新（如输入框打字）中断，从而保持交互流畅

## 19.x

### use API

> 简化在组件中读取资源（如 Promise、Context）的方式

```js
import { Suspense, use } from "react";

// 模拟数据获取函数，返回一个 Promise
const fetchUser = (id) => {
    return fetch(`/api/users/${id}`).then((res) => res.json());
};

// 组件内直接使用 use 消费 Promise
function User({ userId }) {
    // use 会“等待” Promise 完成，期间组件会挂起
...
> ref 作为 Prop：彻底废除了 forwardRef。现在函数组件可以直接通过 ref 参数接收 ref，大大简化高阶组件或库的编写

### 文档元数据组件化

过去，修改页面 <title> 或 <meta> 标签通常需要借助 react-helmet 这类库，或者在路由配置中定义。在 React 19 中，你可以在任何组件里直接写入这些标签
...
React 会自动将这些标签提升到文档的 <head> 中，并确保在服务端渲染时也能正确处理，极大地提升了开发体验和 SEO 便利性
